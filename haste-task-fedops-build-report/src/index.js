...
module.exports = ({base = process.cwd()}) => async () => {
  ...
  const command = ({appName, bundleName}) => {
    const metricNames = {
      ...
      bundle_name: replaceDotsWithUnderscore(path.relative(path.join(base, 'dist/statics'), bundleName)), // eslint-disable-line camelcase
    };
  ...
  const reportBundleSize = params => {
    return new Promise(resolve => {
      return fsStatP(path.resolve(base, params.bundleName))
        .then(stats => {
          ...
          console.log(command(params), stats.size || 0);
  ...
  const config = tryRequire(path.join(base, 'fedops.json'));
